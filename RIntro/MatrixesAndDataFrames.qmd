...
The mode funciton in R refers to storage modes, not the mode of a
distribution.
...
The `as.XXX` and `is.XXX` functions can be used to convert between
different types.
...
R implicitly loops over all the elements of a vector.  Such implicit
loops are faster than explicit `for` loops.
...
The `:` operator produces sequences (of integers) between first and
second argument.
...
-3:-1
```
The `c()` function can be used to glue vectors together.  (`c` stands
for combine.)
...
R has a number of built in random number generators to generate random
numbers.  The most commonly used are `runif`, `rnorm` and `sample`.
There are also many others, with names that look like `rXXX` (try
substituting chisq, t, beta, gamma, &c for XXX).
...
1a.  Use the result of the previous question to generate a random
     sample of size 101 with one outlier of 200.
...
```
3. The variable `state.area` contains the areas of the 50 US states
   (in alphabetical area).  Create a random sample of size 10 of the
   state areas.
...
The `[]` operator is used to subscript vectors.  There are three
different things you can put inside of the brackets: numbers, logical
expressions and names (character values).
...
```
Another useful trick is to use negative indexes.  These leave the
numbered varaibles out.
...
the same length.  For example, `state.abb` gives the two letter postal
codes of the states.  Suppose we wanted to see all of the states that
...
The built in langauge primitive `if` is **not** vectorized.  It is
...
There are two differences.  First the condition is a logical vector.
...
name.  Florida is the `r which(state.abb=="FL")` state alphabetically,
...
Sometimes we need to make up names.  The `paste()` command is handy
for that.  It is vectorized, so you can put a bunch of numbers in.
...
columns. `dim()[1]` gets the number of rows and `dim()[2]` the number
of columns.
...
* A negative number of vector of negative numbers (excluded those rows
  or columns)
* A logical vector of size `nrow(x)` or `ncol(x)` (select the
  rows/columns corresponding to true).
...
To use character indexes with matrixes, we need to set the
`rownames()` and `colnames()` of the matrix.  We can also use the
`dimnames()` (although this will produce a list). 
...
Remember that a matrix is just a vector with a `dim` attribute.
Consequently, `mean` and other summary functions don't do what we
want:
...
The `apply()` operator can turn any summary function into a row or
column function.

```{r}
help(apply)
```
The MARGIN argument to apply should be 1 for rows, 2 for columns and
so forth for generaly arrays. 
...
The `sweep` operator "subtracts" a vector from all of the rows or
columns of the matrix.

"Subtracts" is in quotes because actually any operator can be used
here.  Subtracts "-" and divides "/" are the most common.
...
9. Find the population for all states whose area is bigger than
   Florida's.
...
11. Turn the state.x77 data into z-scores by subtracting the column
    means and dividing by the column standard deviations.

```{r}

```
12. Scale the state.x77 data from 0 (minimum in the column) to 1
