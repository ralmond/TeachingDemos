...
is a distribution often used for waiting times.  Suppose the expected
time to the next arrival is $\theta$.  Then the probability that
person will come at exactly time $x$ is $f(x|\theta) =
\frac{1}{\theta}e^{-x/\theta}$.  The exponential distribution has some
interesting properties.  In particular, if you have already waited for
time period $z$, then the conditional expectation is $z+\theta$.
...
```{=html}
<iframe width="100%" height="500" src="../apps/GammaParams.R"></iframe>
```
